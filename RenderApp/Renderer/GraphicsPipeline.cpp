...
namespace VEE {
	...
	VGraphicsPipeline::VGraphicsPipeline(VDevice* device, VRenderPass* renderPass, VkExtent2D extent) : m_Device(device)
	{
		...
		vkCreatePipelineLayout(m_Device->GetLogicalDeviceHandle(), &pipelineLayoutInfo, nullptr, &m_Layout);
		ASSERT(m_Layout != nullptr, "failed to create Pipeline Layout!");
		...
		ASSERT(m_GraphicsPipeline != nullptr, "failed to create Pipeline");
	}

	void VGraphicsPipeline::Bind(std::shared_ptr<VCommandBuffer>& commandBuffer)
	{
	...
	VGraphicsPipeline::~VGraphicsPipeline()
	{
		...
		vkDestroyPipelineLayout(m_Device->GetLogicalDeviceHandle(), m_Layout, nullptr);
	}
